...
# ╔═╡ 376ac554-04c8-11ee-02d2-1d72147768fb
begin
	using JLD2
	using DataFrames
	using Clustering
...
# ╔═╡ ee673794-1b93-4df1-876d-f49895971725
# Algorithm exploration for Bag of Words + Term Similarity Project

# ╔═╡ 431af055-906c-44b6-8505-2c0aa6651e51
# Each token is converted to an embedding vector via the pretrained word embedding file
...
# ╔═╡ 12a55ee0-0788-41d7-abea-aea800799314
# At a word level, all tokens were lowercased and any token "n't" was marked as Out Of Vocabulary as the splitting of contractions was somewhat inconsistent. Any token marked as an aggregated feature is passed over, we'll touch on this later
...
# ╔═╡ 6d079d21-06af-4287-939e-57c375ea95eb
...
# ╔═╡ 9e11205b-0d27-4415-a28d-679fc699021f
...
# ╔═╡ d07a829d-7199-45e5-82b2-e67795a94397
# Cuts the cluster tree at the provided value of k, calculates solhouette score and mean euclidean distance, and returns the resulting dataframe grouped by cluster.
...
# ╔═╡ 93d8ce55-6eb0-4abf-8702-d8f275abcb7d
# Selects vocabulary in the documents with similarity metrics below provided threshold, maps them to a placeholder feature, and returns a dict mapping of all vocabulary to their aggregated placeholder feature
...
# ╔═╡ 954bcb21-0b9f-4390-ac09-8a66212c605f
# Maps document vocabulary to provided feature dictionary, replacing terms with their aggregated feature where appropriate
...
# ╔═╡ db23ec2a-7ea9-4a44-bbe8-84bec26178d8
# Clustering and documents loaded from pre-computed file for brevity
bitext_dd = JLD2.load("data/bitext_dd_pl.jld2")["data"]

# ╔═╡ 7c9b27db-3275-46cf-9fb8-a241098e849f
# Change the following 3 parameters to change the resulting plot and vocabulary mappings below

# ╔═╡ 4ef50c19-4233-4170-96ff-a380d1d0260f
# From Clustering.jl docs: `k::Integer` (optional) the number of desired clusters
@bind k Slider(2:100, default = 10)

# ╔═╡ 93732af6-7587-4143-81dc-076be097cc0a
# Goodness of fit metric to use
# eucl measures the span of a cluster in the embedding vector space
# sil measures how well each member of a cluster fits with that cluster over other clusters
@bind metric Select(["eucl", "sil"])

# ╔═╡ be8c05a7-95c4-46a4-b0a9-2164c0cf4e29
# Goodness of fit metric value below which clusters will be aggregated to create a placeholder feature in the embedding vocabulary
@bind thresh Slider(0:0.01:1, default = 0.1)

# ╔═╡ a3024623-4fac-49c3-a51d-9fbe15b9e472
# Cluster-grouped dataframe with given k value

bitext_df, bitext_gb = clusters_to_gb(
	bitext_dd["clusts"],
	bitext_dd["vocab"],
	bitext_dd["dists"],
	bitext_dd["coords"],
...
# ╔═╡ d0a5c04a-c688-4711-b513-cae252fa2e36
scatter!(
	bitext_df[!, "umap_x"],
	bitext_df[!, "umap_y"],
	group = bitext_df[!, "cluster"],
	legend = false,
	xlabel = "",
	ylabel = "",
	hover = bitext_df[!, "vocab"],
)

# ╔═╡ e5125549-2bab-4f08-81de-95d9d380a926
# Mapping of vocabulary to aggregated features

feat_dict = combine_features(
	bitext_gb[!, "vocab"], 
	bitext_gb[!, metric], 
	thresh, 
	[])

# ╔═╡ db7b3068-edde-4389-b662-d438c5e7db33
# Docs mapped with aggregated features

feat_docs = replace_features(
	bitext_dd["docs"], 
	feat_dict
)
...
julia_version = "1.9.0"
manifest_format = "2.0"
project_hash = "24aef577ac49da4cb7532fc83c01f943f68f9b3c"
